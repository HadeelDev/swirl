        ...
        if(n < length(choices)){
          repeat {
            ...
            if(is(temp, "try-error")){
              swirl_out(s()%N%"اسف ولكنني لست قادرا على التحميل ", sQuote(choice),
                        ...
                        s()%N%"  قم بزياره مستودع الدوره التدريبيه او 0 للخروج")
              ch <- c(s()%N%"حاول مره اخرى", 
                      s()%N%"ارسل لي اسم مكان الدوره حتى نتمكن من التحميل اليدوي")
              resp <- select.list(ch, graphics=FALSE)
              if(resp == "") return(FALSE)
              if(resp == ch[2]) {#OK. I'm opening the swirl course respository in your browser.
                swirl_out(s()%N%"في متصفحك الخاص swirl ممتاز، انا افتح دورات ")
...
welcome.default <- function(e, ...){ 
  swirl_out(s()%N%"،swirl اهلا بك صديقي في",
            s()%N%" اذا سمحت استخدم اسمك المستعار ،",
            s()%N%"اما اذا كنت ضيفا جديدا فختار لنفسك اسما مميزا باللغه الانجليزيه",skip_after=TRUE)
...
housekeeping.default <- function(e){
  swirl_out(paste0(s()%N%"شكرا لك  ", e$usr, s()%N%"\nللمتابعه Enter اضغط على",
                   s()%N%"\n من الان سوف ننتبه لكل اشاره صغيره" ,
                   s()%N%"\n احضر قهوتك المفضله حتى نستعد سويا للدرس"))
  readline(s()%N%"\n...  <-- هذه اشاره منك لاستمرار")
 
  swirl_out(s()%N%"حتى تستطيع الاستمرار Enter فيجب عليك ان تختار على ",
            s()%N%"عندما تواجه سؤالا يطلب منك ان تختار من تعدد")
  select.list(c(s()%N%"الاستمرار", s()%N%"البرمجه", s()%N%"لنستمر"),
              title=s()%N%"\n ( 1 ,2 or 3 ):اختار احد الارقام التاليه ", graphics=FALSE)
  swirl_out(s()%N%"\n esc بالضغط على  swirl تسطيع الخروج من ",
            s()%N%"\n swirl ثم سوف تشاهد رساله تخبرك بأنك غادرت ")
  info()
...
courseMenu.default <- function(e, choices){
  ...
  swirl_out(s()%N%"swirl اذا سمحت اختار الدوره المناسبه او اختار0 للخروج من ")
